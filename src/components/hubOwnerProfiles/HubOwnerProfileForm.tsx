...
import { type HubOwnerProfile, insertHubOwnerProfileParams } from "@/lib/db/schema/hubOwnerProfiles";
import {
...
const HubOwnerProfileForm = ({
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    const hubOwnerProfileParsed = await insertHubOwnerProfileParams.safeParseAsync({  ...payload });
    ...
    try {
      startMutation(async () => {
        ...
        const error = editing
          ? await updateHubOwnerProfileAction({ ...values, id: hubOwnerProfile.id })
    ...
    <form action={handleSubmit} onChange={handleChange} className={"space-y-8"}>
      {/* Schema fields start */}
        ...
        <Input
          type="text"
          name="companyName"
          className={cn(errors?.companyName ? "ring ring-destructive" : "")}
          defaultValue={hubOwnerProfile?.companyName ?? ""}
        />
        {errors?.companyName ? (
          <p className="text-xs text-destructive mt-2">{errors.companyName[0]}</p>
        ) : (
          <div className="h-6" />
        )}
      </div>
        ...
        <Label
          className={cn(
            "mb-2 inline-block",
            errors?.businessRegistrationNumber ? "text-destructive" : "",
          )}
        >
        ...
        <Input
          ...
          className={cn(errors?.businessRegistrationNumber ? "ring ring-destructive" : "")}
          defaultValue={hubOwnerProfile?.businessRegistrationNumber ?? ""}
        />
        {errors?.businessRegistrationNumber ? (
          <p className="text-xs text-destructive mt-2">{errors.businessRegistrationNumber[0]}</p>
        ) : (
          <div className="h-6" />
        )}
      </div>
        ...
        <Input
          ...
          className={cn(errors?.businessContactNo ? "ring ring-destructive" : "")}
          defaultValue={hubOwnerProfile?.businessContactNo ?? ""}
        />
        {errors?.businessContactNo ? (
          <p className="text-xs text-destructive mt-2">{errors.businessContactNo[0]}</p>
        ) : (
          <div className="h-6" />
        )}
      </div>
        ...
        <Input
          type="text"
          name="businessEmail"
          className={cn(errors?.businessEmail ? "ring ring-destructive" : "")}
          defaultValue={hubOwnerProfile?.businessEmail ?? ""}
        />
        {errors?.businessEmail ? (
          <p className="text-xs text-destructive mt-2">{errors.businessEmail[0]}</p>
        ) : (
          <div className="h-6" />
        )}
      </div>
        ...
        <Input
          ...
          className={cn(errors?.businessLocation ? "ring ring-destructive" : "")}
          defaultValue={hubOwnerProfile?.businessLocation ?? ""}
        />
        {errors?.businessLocation ? (
          <p className="text-xs text-destructive mt-2">{errors.businessLocation[0]}</p>
        ) : (
          <div className="h-6" />
        )}
      </div>
        ...
        <Input
          ...
          className={cn(errors?.residingLocation ? "ring ring-destructive" : "")}
          defaultValue={hubOwnerProfile?.residingLocation ?? ""}
        />
        {errors?.residingLocation ? (
          <p className="text-xs text-destructive mt-2">{errors.residingLocation[0]}</p>
        ) : (
          <div className="h-6" />
        )}
      </div>
        ...
        <Input
          ...
          className={cn(errors?.socialMediaHandles ? "ring ring-destructive" : "")}
          defaultValue={hubOwnerProfile?.socialMediaHandles ?? ""}
        />
        {errors?.socialMediaHandles ? (
          <p className="text-xs text-destructive mt-2">{errors.socialMediaHandles[0]}</p>
        ) : (
          <div className="h-6" />
        )}
      </div>
        ...
        />
        {errors?.websiteUrl ? (
          <p className="text-xs text-destructive mt-2">{errors.websiteUrl[0]}</p>
        ) : (
          <div className="h-6" />
        )}
      </div>
      ...
      {editing ? (
        <Button
          ...
          onClick={() => {
            ...
            closeModal && closeModal();
            startMutation(async () => {
              addOptimistic && addOptimistic({ action: "delete", data: hubOwnerProfile });
              const error = await deleteHubOwnerProfileAction(hubOwnerProfile.id);
              setIsDeleting(false);
              const errorFormatted = {
                error: error ?? "Error",
